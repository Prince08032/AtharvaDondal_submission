...
export const Query = {
  ...
  myAssets: async (
    _: any,
    { after, first = 20, q }: { after?: string; first?: number; q?: string },
    { supabaseUser, supabaseAdmin, user }: GraphQLContext
  ) => {
    requireAuth(user);
    ...
    if (!supabase) {
      throw new GraphQLError("Supabase client is not initialized", {
    ...
    const edges = items.map((asset: any) => ({
  ...
  getDownloadUrl: async (
    ...
    if (finalAsset.status !== "ready") {
      throw new GraphQLError(
        `Asset not ready for download (status: ${finalAsset.status})`,
        { extensions: { code: "BAD_REQUEST" } }
      );
    }
